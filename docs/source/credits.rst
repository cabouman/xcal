Credits
=======

XCAL Publication
---------------------

For details on the XCAL algorithm, see the following:

    Wenrui Li, K. Aditya Mohan, Venkatesh Sridhar, Xin Liu, Jean-Baptiste Forien, Joseph Bendahan, Saransh Singh, Gregery T. Buzzard, and Charles A. Bouman, "XCal: model-based approach to X-ray CT spectral calibration," `Opt. Express, vol. 33, no. 15, pp. 30875-30896, July 2025 <https://opg.optica.org/oe/fulltext.cfm?uri=oe-33-15-30875>`_.

XCAL Development Team
---------------------

The XCAL Development Team is listed below:

    Wenrui Li, K. Aditya Mohan, Venkatesh Sridhar, Xin Liu, Jean-Baptiste Forien, Joseph Bendahan, Saransh Singh, Gregery T. Buzzard, Charles A. Bouman


XCAL Sponsors
-------------

We would like to thank the following sponsors for their financial support in the development of this python package:

    * The Laboratory Directed Research and Development program of Lawrence Livermore National Laboratory under project number 22-ERD-011.
    * The Ralph W. and Grace M. Showalter Research Trust Fund.


Citation
--------

Please use the following Bibtex citation when referencing this software.
::

    @Misc {xcal,
    author = {Wenrui Li and K. Aditya Mohan and Venkatesh Sridhar and
              Xin Liu and Jean-Baptiste Forien and Joseph Bendahan and
              Saransh Singh and Gregery T. Buzzard and Charles A. Bouman},
    title =	{{X-ray Spectrum Calibration}},
    howpublished = {Software library available from \url{https://github.com/cabouman/xcal}},
    note = {Version 0.2.1},
    year = 2026
    }

