...
if __name__ == '__main__':  
    ...
    for rand_seed_num in [71, 81]:
        ...
        simkV_list_indices = np.array([3, 11])
        ...
        for simkV, gt_spec_dict in zip(simkV_list[simkV_list_indices],gt_spec_simkv_list):
            ...
            plt.legend()
            plt.ylim((0,1))
            plt.savefig('./output_exp15/exp3_%s_proj_sv%d_%d.png' % (filename,simkV, rand_seed_num))
        ...
        num_threads = 8
        src_response_list = [np.array([src_spec_list[sli]]) for sli in simkV_list_indices]
        res, params_combinations = xspec.parallel_anal_sep_model(num_threads,
            ...
            energies, simkV_signal_train_list, spec_F_train,
            fltr_th_bound=(0, 10),
            scint_th_bound=(0.02, 0.5),
            learning_rate=0.2, iterations=2000, tolerance=1e-5, return_history=False)

        np.save('./output_exp15/exp3_res_rdsd_%d.npy'%(rand_seed_num), np.array([res, params_combinations], dtype=object))
