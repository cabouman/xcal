...
def _compute_criteria_and_beta(yhat_F, spec_dict, energies, y_yhat, FDsq, y_F_Dk, yhat_sq, S, auto_stop, ysq, signal):
    ...
    # Compute yhat_F_Dk
    yhat_F_Dk = np.trapzoid(yhat_F * spec_dict, energies, axis=0)
...
def dictSE(signal, energies, forward_mat, spec_dict, src_info, fltr_info_dict, scints_info_dict,
    ...
    #         sp_als/= np.trapzoid(sp_als,energies) # normalized spectrum response.
    ...
    #
    #         # Poly-energic
    #         polyE_projs = np.trapzoid(np.exp(-2*wnum*beta_projs.transpose((0,2,3,4,1)))*sp_als,energies, axis=-1)
    #
    #         # Add noise
    ...
    y_F_Dk = np.trapzoid(y_F * spec_dict, energies, axis=0)
    Fsq = np.einsum('ik,k,kj->ij', forward_mat.T, signal_weight.flatten(), forward_mat)

    D_Fsq = np.trapzoid(spec_dict.T[:, :, np.newaxis] * Fsq[np.newaxis, :, :], energies, axis=1)
    if verbose > 0:
        print('D_Fsq shape:', D_Fsq.shape)
    FDsq = np.trapzoid(D_Fsq * spec_dict.T, energies, axis=1)
    if verbose > 0:
    ...
    while len(S_list) > 0:
        ...
        for tst_ind, S, omega, pre_FDS in zip(np.arange(len(S_list)), S_list, omega_list, FDS_list):
            ...
            for cl in candidate_list:
                ...
                print(new_S)
                FDk = np.trapzoid(forward_mat[:, :, np.newaxis] * spec_dict[np.newaxis, :, k], energies, axis=1)
...
def uncertainty_analysis(signal, back_ground_area, energies, forward_mat, spec_dict, sparsity, num_candidate=1,
    ...
    if anal_mode == 'add_noise_to_signal':
        ...
        estimated_spec, omega, S, cost_list = dictSE(signal, energies, forward_mat,
                                                     ...
                                                     return_component=True,
                                                     verbose=0)
        ideal_proj = [np.trapzoid(fwm * estimated_spec.flatten(), energies, axis=-1).reshape(sig.shape) for sig, fwm in
...
def dictSE_sep_model(signal, energies, forward_mat,
    ...
    Z = np.trapzoid(spec_dict, energies, axis=0).reshape((1, spec_dict.shape[-1]))
    ...
    for i_src in range(src_len):
        for i_fltr in range(fltr_len):
            for i_scint in range(scint_len):
                ...
                FDS = np.trapzoid(forward_mat[:, :, np.newaxis] * spec_dict[np.newaxis, :, SS], energies, axis=1)
