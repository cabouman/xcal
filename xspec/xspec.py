...
class LS:
    ...
    def solve(self, FD, y, Z, weight=None, Omega_init=None, e_init=None):
        """Use pairwise ICD to solve MAP cost function with Snap prior.
        ...
        for K in range(self.max_iter):
            ...
            for pair_ind in pairs[perm]:
                ...
                omega_tmp_k = Omega[k].copy()
                omega_tmp_g = Omega[g].copy()
                theta_1 = (e * weight).T @ (yZ_FD[:, k:k + 1] - yZ_FD[:, g:g + 1]) / m
                theta_2 = ((yZ_FD[:, k:k + 1] - yZ_FD[:, g:g + 1]) * weight).T @ (
                        yZ_FD[:, k:k + 1] - yZ_FD[:, g:g + 1]) / m

                a = Z[0, k] - Z[0, g]
                b = Z @ Omega
                c = theta_2
                d = theta_1*2
                e1 = ((yZ_FD@Omega).reshape((-1,1)) * weight).T @ (yZ_FD@Omega).reshape((-1,1)) / m
                ...
                e = e + (yZ_FD[:, k:k + 1] - yZ_FD[:, g:g + 1]) * (Omega[k] - omega_tmp_k)
...
# Orthogonal match pursuit with different optimization models.
def dictSE2(signal, energies, forward_mat, spec_dict, src_info, fltr_info_dict, scints_info_dict,
           sparsity, optimizor, num_candidate=1, max_num_supports=None,
           signal_weight=None, tol=1e-6,
           return_component=False, auto_stop=True, apply_phys_constraint=True, apc_method='traverse',
           verbose=0):
    ...
    while len(S_list) > 0:
        ...
        for tst_ind, S, omega, pre_FDS in zip(np.arange(len(S_list)), S_list, omega_list, FDS_list):
            ...
            yZFDsq_omega = yZFDsq @ omega
            yZFD_omegasq = omega.T @ yZFDsq @ omega
            print('Min yZFDsq_omega', np.min(yZFDsq_omega))
            print('Min yZFD_omegasq', np.min(yZFD_omegasq))
            # Compute rho1 and rho2
            rho1 = diag_yZFDsq - yZFDsq_omega
            rho2 = diag_yZFDsq - 2 * yZFDsq_omega + yZFD_omegasq
            ...
            if verbose>0:
                print('b*(d+2*e)-2*a*e', b*(d+2*e1)-2*a*e1)
                print('a*d-b*(2*c+d)', a*d-b*(2*c+d))

                print('yZFDsq_omega shape:', yZFDsq_omega.shape)
                print('yZFD_omegasq shape:', yZFD_omegasq.shape)
                print('rho1 shape:', rho1.shape)
                print('rho2 shape:', rho2.shape)
                print('min of rho2:', np.min(rho2))
                rho2_argmin = np.argmin(rho2)
                print('diag_yZFDsq[rho2_argmin]:',diag_yZFDsq[rho2_argmin])
                print('yZFDsq_omega[rho2_argmin]:',yZFDsq_omega[rho2_argmin])
                print('yZFD_omegasq:',yZFD_omegasq)
            ...
            # Calculate criteria
            criteria = beta ** 2 * yZFD_omegasq + (1 - beta) ** 2 * diag_yZFDsq + 2 * beta * (1 - beta) * yZFDsq_omega
            criteria_norm = ((Z@(omega*beta.flatten()[np.newaxis, :]+np.diag(1-beta.flatten()))).T)**2
            ...
            if criteria.mask.all():
                ...
                res_omega_list.append(omega)
                res_estimated_spec_list.append(estimated_spec)
                res_cost_list.append(cal_cost(e/(Z[:,S]@ new_omega[S]), signal_weight))
                continue
            elif criteria.count() >= num_candidate:
                candidate_list = np.argsort(criteria)[:num_candidate]
            else:
            ...
            for cl in candidate_list:
                ...
                print('Dictionary element index:', k)
                print('beta_k:', beta[k])
                print('criteria k:', criteria[k])
                ...
                # Find best coefficient with new support given solver.
                Omega_init = new_omega[new_S, 0]
                e = (yZ[:,new_S] - FDS) @ new_omega[new_S]#/(Z[:,new_S]@ new_omega[new_S])

                new_omega[new_S, 0] = optimizor.solve(FDS, signal, Z[:,new_S], weight=signal_weight, Omega_init=Omega_init,
                                                      e_init=e)
                e = (yZ[:,new_S] - FDS) @ new_omega[new_S]

                if len(new_S) == sparsity or np.linalg.norm(e/(Z[:,new_S]@ new_omega[new_S])) <= tol:
                    res_S_list.append(new_S)
                    res_omega_list.append(new_omega)
                    res_estimated_spec_list.append(estimated_spec)
                    res_cost_list.append(cal_cost(e/(Z[:,new_S]@ new_omega[new_S]), signal_weight))
                else:
                    new_S_list.append(new_S)
                    new_omega_list.append(new_omega)
                    new_FDS_list.append(FDS)
                    cost = optimizor.cost(Z, new_omega)
                    new_cost_list.append(cost)
                    print('cost:', cost)
...
def dictSE_sep_model(signal, energies, forward_mat,
    ...
    while len(S_src)<sparsity_src or len(S_fltr)<sparsity_fltr or len(S_scint) < sparsity_scint:
        ...
        print('omega_src:', omega_src[S_src])
